...
describe("New Transaction", function () {
  ...
  it("navigates to the new transaction form, selects a user and submits a transaction payment", function () {
    ...
    const updatedAccountBalance = toDecimal(dinero({ amount: ctx.user!.balance - parseInt(payment.amount) * 100, currency: USD }), ({ value }) =>
  ...
  it("submits a transaction payment and verifies the deposit for the receiver", function () {
    ...
    const updatedAccountBalance = toDecimal(dinero({ amount: ctx.contact!.balance + transactionPayload.amount * 100, currency: USD }), ({ value }) =>
      Number(value).toLocaleString("en-US", { style: "currency", currency: "USD" })
    );
  ...
  it("submits a transaction request and accepts the request for the receiver", function () {
    ...
    const updatedAccountBalance = toDecimal(dinero({ amount: ctx.user!.balance + transactionPayload.amount * 100, currency: USD }), ({ value }) =>
      Number(value).toLocaleString("en-US", { style: "currency", currency: "USD" })
    );
