...
export const transactionDetailMachine = dataMachine("transactionData").withConfig({
  services: {
    ...
    createData: async (ctx, event: any) => {
      ...
      const resp = await httpClient.post(`http://localhost:3001/${route}/${payload.transactionId}`, payload);
      return resp.data;
    },
    updateData: async (ctx, event: any) => {
      ...
      const resp = await httpClient.patch(`http://localhost:3001/transactions/${transactionId}`, payload);
      return resp.data;
    },
  },
});
