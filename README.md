...
> If you want to change the default ports, you can do so by modifying `VITE_PORT` and `VITE_BACKEND_PORT` variables in `.env` file.
...
> The port number in `baseUrl` corresponds to `VITE_PORT` variable in `.env` file. Similarly, the port number in `apiUrl` and `url` correspond to `VITE_BACKEND_PORT`.
> For example, if you have changed `VITE_PORT` to `13000` and `VITE_BACKEND_PORT` to `13001` in `.env` file, then your `cypress.json` should look similar to the following snippet:
>
> ```json
> {
